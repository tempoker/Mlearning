#在误卸载anaconda之后第一个小项目 2018-12-12 21:05
...
root_train,root_test = r'train.csv',r'test.csv'
...
def train_circle():
	...
	for epoch in range(n_epochs):
		for batch_idx,(img,label) in enumerate(train_loader,start=1):
			...
			#print(out,label,sep='\n') #交叉熵 -sum(pi*logqi)
			acc = correct / count #pi:真实值 qi:预测概率值
		...
		# if epoch ==2:
		# 	plt.plot(train_batch,train_acc)
		# 	plt.show()
...
if __name__ == "__main__":
	train_circle()
	test_criRec()
''' #out like follows:  id	    y
[ 0.4369,  0.3932],     4000	1
[ 0.4150,  0.5042],     4001	0
pred_res= [0,1] 按行取较大者的索引作为预测值
'''